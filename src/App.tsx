...
export default function App() {
  ...
  // AUTO-LOADER: Preloads the base STL on startup
  useEffect(() => {
    if (!debugInfo.ready || !sceneRef.current || baseMesh) return;
    ...
    if (intersects.length > 0) {
      ...
      if (snapToGrid) {
        ...
        if (baseMesh) {
          ...
          const meshIntersects = checkRaycaster.intersectObject(baseMesh);
          if (meshIntersects.length > 0) return { x: snappedX, y: snappedY };
          else return null;
        } else {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (isDrawing && currentWall) {
      ...
      setWalls(prev => prev.map(w => {
        if (w.id === selectedWallId) {
          ...
          if (baseMesh) {
            ...
            if (checkRaycaster.intersectObject(baseMesh).length === 0) return w;
      ...
      if (hoveredHole) { setHoveredId(hoveredHole.id); return; }
      const hoveredWall = walls.find(w => getDistanceToWall(point, w) < 5);
      if (hoveredWall) { setHoveredId(hoveredWall.id); return; }
      setHoveredId(null);
    }
  };
  ...
  const exportSTL = async () => {
    ...
    try {
      ...
      if (baseMesh) {
        ...
        if (!baseMeshToProcess.geometry.index) baseMeshToProcess.geometry = mergeVertices(baseMeshToProcess.geometry);
      ...
      if (baseMeshToProcess && holes.length > 0) {
        ...
        for (const hole of holes) {
          ...
          try { currentResult = CSG.subtract(currentResult, holeMesh); } 
      ...
      <header className="flex items-center justify-between px-6 py-4 border-b border-neutral-200 z-20 bg-white">
        ...
        <div className="flex items-center gap-4">
          <div className="flex bg-neutral-100 p-1 rounded-xl border border-neutral-200">
            <button onClick={() => setActiveTool('select')} className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all ${activeTool === 'select' ? 'bg-white shadow-sm text-blue-600' : 'text-neutral-500 hover:text-neutral-700'}`}><MousePointer2 className="w-4 h-4" /><span className="text-sm font-semibold">Select</span></button>
            <button onClick={() => { setActiveTool('draw'); if (cameraRef.current && controlsRef.current) { cameraRef.current.position.set(0, 150, 0); controlsRef.current.target.set(0, 0, 0); controlsRef.current.update(); }}} className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all ${activeTool === 'draw' ? 'bg-white shadow-sm text-blue-600' : 'text-neutral-500 hover:text-neutral-700'}`}><PenTool className="w-4 h-4" /><span className="text-sm font-semibold">Walls</span></button>
            <button onClick={() => { setActiveTool('hole'); if (cameraRef.current && controlsRef.current) { cameraRef.current.position.set(0, 150, 0); controlsRef.current.target.set(0, 0, 0); controlsRef.current.update(); }}} className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all ${activeTool === 'hole' ? 'bg-white shadow-sm text-blue-600' : 'text-neutral-500 hover:text-neutral-700'}`}><Circle className="w-4 h-4" /><span className="text-sm font-semibold">Hole</span></button>
          </div>
          <button onClick={undo} disabled={history.length === 0} className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all border ${history.length > 0 ? 'bg-white border-neutral-200 text-neutral-700 shadow-sm' : 'bg-neutral-50 border-neutral-100 text-neutral-300 cursor-not-allowed'}`} title="Undo (Ctrl+Z)"><Undo2 className="w-4 h-4" /><span className="text-sm font-semibold">Undo</span></button>
          <button onClick={exportSTL} disabled={isExporting} className="flex items-center gap-2 px-6 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white rounded-xl font-bold transition-all shadow-md active:scale-95">{isExporting ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Download className="w-4 h-4" />}<span>{isExporting ? 'Processing...' : 'Export STL'}</span></button>
        </div>
      </header>
      ...
      <main className="flex flex-1 relative overflow-hidden">
        <aside className="w-72 bg-white border-r border-neutral-200 flex flex-col z-20 shadow-xl">
          <div className="p-6 space-y-8 overflow-y-auto">
            {/* Note: Base model loading is now automatic and section removed from UI */}
            <section className="space-y-4">
              <h2 className="text-[10px] font-black text-neutral-400 uppercase tracking-widest">Wall Specs</h2>
              <div className="grid grid-cols-2 gap-3">
                <div className="p-4 bg-neutral-50 rounded-2xl border border-neutral-100"><p className="text-[9px] font-black text-neutral-400 uppercase">Thickness</p><p className="text-lg font-mono font-bold text-neutral-800">{WALL_THICKNESS}mm</p></div>
                <div className="p-4 bg-neutral-50 rounded-2xl border border-neutral-100"><p className="text-[9px] font-black text-neutral-400 uppercase">Height</p><p className="text-lg font-mono font-bold text-neutral-800">{WALL_HEIGHT}mm</p></div>
              </div>
            </section>

            <section className="space-y-4">
              <h2 className="text-[10px] font-black text-neutral-400 uppercase tracking-widest">Editor</h2>
              {activeTool === 'select' && <div className="p-4 bg-blue-50 border border-blue-100 rounded-2xl flex gap-3 animate-in fade-in slide-in-from-top-2 duration-300"><Info className="w-4 h-4 text-blue-500 shrink-0 mt-0.5" /><p className="text-[11px] leading-relaxed text-blue-700 font-medium"><strong className="block mb-0.5">Select Mode</strong>Click and drag walls or holes to move them.</p></div>}
              <div className="space-y-2">
                <label className="flex items-center justify-between p-4 bg-neutral-50 rounded-2xl border border-neutral-100 cursor-pointer hover:bg-neutral-100 transition-all"><span className="text-xs font-bold text-neutral-600">Snap to Grid</span><input type="checkbox" checked={snapToGrid} onChange={(e) => setSnapToGrid(e.target.checked)} className="w-4 h-4 text-blue-600 rounded-full" /></label>
                <label className="flex items-center justify-between p-4 bg-neutral-50 rounded-2xl border border-neutral-100 cursor-pointer hover:bg-neutral-100 transition-all"><span className="text-xs font-bold text-neutral-600">Show Grid</span><input type="checkbox" checked={gridVisible} onChange={(e) => { setGridVisible(e.target.checked); if (gridHelperRef.current) gridHelperRef.current.visible = e.target.checked; }} className="w-4 h-4 text-blue-600 rounded-full" /></label>
              </div>
              {showClearConfirm ? <div className="flex gap-2"><button onClick={clearWalls} className="flex-1 p-4 bg-red-600 text-white rounded-2xl font-bold text-xs active:scale-95 transition-transform">Confirm Clear</button><button onClick={() => setShowClearConfirm(false)} className="p-4 bg-neutral-200 text-neutral-600 rounded-2xl font-bold text-xs">Cancel</button></div> : <button onClick={() => setShowClearConfirm(true)} className="w-full flex items-center justify-center gap-2 p-4 bg-red-50 hover:bg-red-100 text-red-600 rounded-2xl border border-red-100 transition-all font-bold text-xs active:scale-95"><Trash2 className="w-4 h-4" /><span>Clear All</span></button>}
            </section>

            <section className="pt-6 border-t border-neutral-100">
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between"><div className="flex items-center gap-2"><PenTool className="w-3 h-3 text-neutral-400" /><span className="text-xs font-bold text-neutral-400">Walls: <span className="text-neutral-900">{walls.length}</span></span></div><div className="flex items-center gap-2"><Circle className="w-3 h-3 text-neutral-400" /><span className="text-xs font-bold text-neutral-400">Holes: <span className="text-neutral-900">{holes.length}</span></span></div></div>
                {(selectedHoleId || selectedWallId) && <button onClick={deleteSelected} className="w-full flex items-center justify-center gap-2 p-3 bg-red-50 text-red-600 rounded-xl border border-red-100 font-bold text-xs hover:bg-red-100 transition-all mt-2 active:scale-95"><Trash2 className="w-4 h-4" />Delete Selected {selectedHoleId ? 'Hole' : 'Wall'}</button>}
              </div>
            </section>
          </div>
        </aside>
        ...
        <div className="flex-1 relative bg-[#f8f9fa] border-2 border-red-500 overflow-hidden">
          <div ref={containerRef} className="absolute inset-0" style={{ cursor: hoveredId ? 'pointer' : (activeTool === 'draw' ? 'crosshair' : (activeTool === 'select' ? 'default' : 'grab')) }} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} />
          <div className="absolute top-4 right-4 bg-black/80 text-white p-2 rounded text-xs font-mono z-50 pointer-events-none">Viewport: {debugInfo.width}x{debugInfo.height} {debugInfo.ready ? '(Ready)' : '(Wait)'} | F: {debugInfo.frames}</div>
          <div className="absolute top-6 right-6 flex flex-col gap-2 z-10"><div className="bg-white/80 backdrop-blur-md p-2 rounded-2xl border border-neutral-200 shadow-xl flex flex-col gap-1"><button onClick={() => { if (cameraRef.current && controlsRef.current) { cameraRef.current.position.set(0, 150, 0); controlsRef.current.target.set(0, 0, 0); controlsRef.current.update(); }}} className="p-3 hover:bg-blue-50 rounded-xl text-neutral-500 transition-all" title="Top View"><Layers className="w-5 h-5" /></button><button onClick={() => { if (cameraRef.current && controlsRef.current) { cameraRef.current.position.set(100, 100, 100); controlsRef.current.target.set(0, 0, 0); controlsRef.current.update(); }}} className="p-3 hover:bg-blue-50 rounded-xl text-neutral-500 transition-all" title="Perspective View"><Box className="w-5 h-5" /></button></div></div>
          <AnimatePresence>{(activeTool === 'draw' || activeTool === 'hole') && <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="absolute bottom-10 left-1/2 -translate-x-1/2 bg-blue-600 text-white px-8 py-4 rounded-2xl shadow-2xl flex items-center gap-4 pointer-events-none z-10">{activeTool === 'draw' ? <PenTool className="w-5 h-5" /> : <Circle className="w-5 h-5" />}<span className="text-sm font-black uppercase tracking-widest">{activeTool === 'draw' ? 'Wall Drawing Active' : 'Hole Placement Active'}</span><div className="w-2 h-2 bg-white rounded-full animate-pulse" /></motion.div>}</AnimatePresence>
        </div>
      </main>
    </div>
  );
